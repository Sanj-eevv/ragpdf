...
| Cross-Encoder re-ranking | `app/Services/Retrieval/ChunkReranker.php` via `Laravel\Ai\Reranking` (Jina) | **Conceptually yes, mechanically different — see divergences below** |
...
### 2. The re-ranker is a hosted API, not a self-hosted open-source cross-encoder
The thesis says "a Cross-Encoder re-ranking model is applied" without naming a specific model or hosting approach. This implementation uses `Laravel\Ai\Reranking` backed by **Jina's rerank API** (a genuine cross-encoder architecture under the hood, so the *technique* claim holds), rather than a locally-run open-source model like `cross-encoder/ms-marco-MiniLM-L-6-v2`. This was a deliberate, explicit choice you approved mid-build (traded a self-hosted Python/sentence-transformers sidecar for a hosted API, on cost/simplicity grounds — Jina is effectively free at this project's scale).

**Why this matters for your write-up**: the thesis's "localized, cost-effective" framing (Chapter 1.1, 3.2.2) is specifically about *not needing a separate vector database service* — it was never about avoiding LLM API calls entirely (embeddings/generation/judging were always going via OpenAI's cloud API in the thesis's own design). Adding Jina as a *second* external API dependency is a minor addition to that external-dependency surface, not a contradiction of the "localized" claim, but your methodology chapter should name Jina explicitly rather than imply a self-hosted cross-encoder, since that's what was actually run.

### 3. The implementation uses Laravel's official AI SDK, not raw OpenAI HTTP calls
The thesis's Chapter 3.2/3.3 reads as if the OpenAI API is called fairly directly/manually ("Laravel immediately converts it into an embedding using the OpenAI API"). This build uses `laravel/ai` (Laravel's first-party AI SDK, `Laravel\Ai\Embeddings`/`Reranking`/`Agent` classes) rather than hand-rolled HTTP client calls. The underlying behavior verified against the thesis's literal SQL/API mechanics is identical (same models, same embedding dimensions, same pgvector operators) — this is an implementation-detail difference, not a behavioral one. Worth naming the actual tool in your methodology chapter (`laravel/ai`) rather than describing bespoke HTTP glue code, since a reader who goes looking at the code should recognize what you describe.
